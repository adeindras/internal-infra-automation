...
String SERVICE_NAME
String JOB_INIT
def ENV_PATH = EKS_CLUSTER_NAME.replace('-','/')
...
node('init-deploy') {
    container('jnlp') {
        stage('Init') {
            ...
            echo "EKS Cluster Name: ${params.EKS_CLUSTER_NAME}"
            echo "EKS Cluster Region: ${params.EKS_CLUSTER_REGION}"

            // Extract CUSTOMER, SERVICE_NAME, and ENVIRONMENT from CLUSTER_NAME
            CUSTOMER = params.EKS_CLUSTER_NAME.split('-')[0]
            SERVICE_NAME = params.EKS_CLUSTER_NAME.split('-')[1]
            ENVIRONMENT = params.EKS_CLUSTER_NAME.split('-')[2]
            
            // Display build name with the extracted values
            currentBuild.displayName = "#${BUILD_NUMBER}-obv-functionality-${params.EKS_CLUSTER_NAME}"
        ...
        try {
            ...
            stage("Checking Otel") {
                ...
                withCredentials([gitUsernamePassword(credentialsId: 'prometheus-api-central-stack', gitToolName: 'Default')]) {
                    ...
                    def SpanQuery = 'query=(clamp_min(sum(rate(otelcol_receiver_accepted_spans_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver),1)/clamp_min((sum(rate(otelcol_receiver_refused_spans_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver))+(sum(rate(otelcol_receiver_accepted_spans_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver)),1)) * 100'
                    def FinalSpanQuery = SpanQuery.replace('EKS_CLUSTER_NAME', "${env.EKS_CLUSTER_NAME}")
                    ...
                    def MetricsQuery = 'query=(clamp_min(sum(rate(otelcol_receiver_accepted_metric_points_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver),1)/clamp_min((sum(rate(otelcol_receiver_refused_metric_points_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver))+(sum(rate(otelcol_receiver_accepted_metric_points_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver)),1)) * 100'
                    def FinalMetricsQuery = MetricsQuery.replace('EKS_CLUSTER_NAME', "${env.EKS_CLUSTER_NAME}")
                    ...
                    def LogsQuery = 'query=(clamp_min(sum(rate(otelcol_receiver_accepted_log_records_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver),1)/clamp_min((sum(rate(otelcol_receiver_refused_log_records_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver))+(sum(rate(otelcol_receiver_accepted_log_records_total{environment_name=\"EKS_CLUSTER_NAME\", job="opentelemetry-collector"}[4m])) by (receiver)),1)) * 100'
                    def FinalLogsQuery = LogsQuery.replace('EKS_CLUSTER_NAME', "${env.EKS_CLUSTER_NAME}")
            ...
            stage("Checking KubeStateMetrics") {
                ...
                withCredentials([gitUsernamePassword(credentialsId: 'prometheus-api-central-stack', gitToolName: 'Default')]) {
                    ...
                    def KSMQuery = 'query=count by (environment_name)({job=\"metric-services/infra\", environment_name=\"EKS_CLUSTER_NAME\", __name__=~\"kube_.*\"})'
                    def FinalKSMQuery = KSMQuery.replace('EKS_CLUSTER_NAME', "${env.EKS_CLUSTER_NAME}")
                    def CurrentTs = sh(script: 'date +%s', returnStdout: true).trim().toLong()
                    def FivemAgoTs = CurrentTs-300
            ...
            stage("Checking Fluentd") {
                ...
                withCredentials([gitUsernamePassword(credentialsId: 'prometheus-api-central-stack', gitToolName: 'Default')]) {
                    ...
                    echo "${CurrentTime}"
                    def LogsPath = "s3://${EKS_CLUSTER_NAME}-logs/logs/access/${CurrentTime}"
            ...
            stage('Results') {
                ...
                withCredentials([string(credentialsId: "ab-deploy-automation-slackbot-token", variable: 'slackToken')]) {
                    pushReport(slackChannel, slackMessage, slackToken)
                        ...
                        postData.blocks[1].fields[3].text = postData.blocks[1].fields[3].text.replace("Running :loading2:", "Finished :done-stamp:")
