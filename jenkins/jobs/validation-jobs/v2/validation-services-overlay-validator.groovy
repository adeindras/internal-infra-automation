                    ...
                    dir("deployments${BUILD_NUMBER}") {
                        def cmd = """#!/bin/bash
                            ...
                            writeTableToFile() {
                                ...
                                printSignature() {
                                    echo "Generated with ❤︎ by Rightsize Team"
                                }
...
void updateSlackStatus(String serviceName, status, token) {
    ...
    if (status == "SUCCESS" || status == "INPROGRESS") {
        linkUrl = "${BUILD_URL}Diff_20Report/"
        linkText = "[REPORT]"
        separator = " "
        ...
        linkUrl = "${BUILD_URL}console"
        linkText = "check this"
        separator = "   ----->    "
    }

    def messageText = "• ${serviceName}: :${slackEmoji}:${separator}<${linkUrl}|*${linkText}*>"

    postData = [
        channel: slackChannel,
        text: "${serviceName} status update: ${status}",
        blocks: [
            [
                type: "section",
                text: [
