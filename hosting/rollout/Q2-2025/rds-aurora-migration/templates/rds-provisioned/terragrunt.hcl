...
inputs = {
  ...
  # ssm_rds_username_path = format("/eks/%s/%s/%s/postgres_username_sandbox", local.customer_name, local.project_name, local.environment_name)
  # ssm_rds_password_path = format("/eks/%s/%s/%s/postgres_password_sandbox", local.customer_name, local.project_name, local.environment_name)
  ...
  route53_name                   = "rds-aurora-justice-shared-prov.${dependency.dns_zone.outputs.route53_internal_zone_name}"
}
