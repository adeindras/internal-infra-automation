...
String taskTitle = "Upgrade MSK cluster"
String taskDesc = "Upgrade MSK cluster to use Kafka version 3.9.x"
String taskImpact = "During the upgrade process, partitions will be reassigned to the consumers as the brokers are being upgraded. The clients are expected to automatically reconnect and gracefully handle this event. No direct impact to the players. Messages published during the upgrade may be lost. "
...
properties(
  [
    parameters([
      ...
      string(name: 'TG_PATH', defaultValue: 'msk/justice-shared', description: 'Path to MSK terragrunt module to be updated'),
      booleanParam(name: 'PLAN_ONLY', defaultValue: true, description: 'Only generate the plan, without executing it')
    ])
  ]
)
    ...
    stage('Plan changes') {
      ...
      steps {
        dir("$WORKSPACE/hosting/rollout/Q2-2025/msk-upgrade") {
          sh "bash update_terragrunt.sh ${params.TG_PATH}"
        }

        dir("${WORKSPACE}/iac/live/${AWS_ACCOUNT}/${CUSTOMER_NAME}/${PROJECT}/${AWS_REGION}/${ENVIRONMENT_NAME}/${params.TG_PATH}") {
          script {
            runTerragrunt "plan"
          }
        }
        ...
        script {
          successJobReply( "Terragrunt plan is completed" )
        }
      }
    }

    stage('Apply changes') {
      when { expression { params.TARGET_ENVIRONMENT_NAME != 'blank' && !params.PLAN_ONLY } }
      steps {
        ...
        script {
          successJobReply( "Terragrunt apply is completed" )
        }
      }
    }

    stage('Push changes & create PR') {
      when { expression { params.TARGET_ENVIRONMENT_NAME != 'blank' && !params.PLAN_ONLY } }
      steps {
        dir("${WORKSPACE}/iac/live/${AWS_ACCOUNT}/${CUSTOMER_NAME}/${PROJECT}/${AWS_REGION}/${ENVIRONMENT_NAME}/${params.TG_PATH}") {
          script {
            BB_BRANCH_NAME = "jenkins-${env.AWS_ACCOUNT}-${TARGET_ENVIRONMENT_NAME}-msk-upgrade-${BUILD_NUMBER}"
            sshagent(['bitbucket-repo-read-only']) {
              sh """#!/bin/bash
                  ...
                  git commit -m "feat: Upgrade MSK kafka version to 3.9.x ${TARGET_ENVIRONMENT_NAME}"
            ...
            bitbucketModule.createPR("MSK Upgrade 3.9.x", "${TARGET_ENVIRONMENT_NAME}", "${BB_BRANCH_NAME}")
          }
        }
