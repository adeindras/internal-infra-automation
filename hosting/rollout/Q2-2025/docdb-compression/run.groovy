...
properties(
    [
        parameters([
            ...
            booleanParam(description: 'Run in dry run mode?', name: 'DRY_RUN', defaultValue: true),
...
stage('Preparation') {
    ...
    stage('Get Database Credentials for Shared DocDB') {
      ...
      steps {
        // Get shared docdb parameters, write it to jumpbox/manifests/mongo.env for kustomize to load as a Kubernetes secret
    ...
    stage('Run in dry-run mode') {
      ...
      steps {
        dir("$WORKSPACE/hosting/rollout/Q2-2025/docdb-compression/scripts") {
          sh 'bash run.sh --dry-run'
        }
      }
    }

    stage('Enable DocDB Compression') {
      when { 
        expression { params.TARGET_ENVIRONMENT_NAME != 'blank' }
        expression { ! params.DRY_RUN }
      }
