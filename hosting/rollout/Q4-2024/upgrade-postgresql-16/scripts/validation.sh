...
compare_databases() {
    ...
    local new_dbs=$(run_sql $NEW_HOST "SELECT datname FROM pg_database WHERE datistemplate = false;")
...
compare_tables_and_rows() {
    ...
    for db in $dbs; do
        ...
        local new_tables=$(run_sql $NEW_HOST "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public';" "$db")
        ...
        for table in $old_tables $new_tables; do
            ...
            local new_rows=$(run_sql $NEW_HOST "SELECT COUNT(*) FROM \"$table\";" "$db")
...
compare_indexes() {
    ...
    for db in $dbs; do
        ...
        local new_indexes=$(run_sql $NEW_HOST "SELECT indexname FROM pg_indexes WHERE schemaname = 'public';" "$db")
...
compare_functions() {
    ...
    for db in $dbs; do
        ...
        local new_functions=$(run_sql $NEW_HOST "SELECT proname FROM pg_proc WHERE pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');" "$db")
...
compare_sequences() {
    ...
    for db in $dbs; do
        ...
        local new_sequences=$(run_sql $NEW_HOST "SELECT sequence_name, start_value, increment, maximum_value, minimum_value FROM information_schema.sequences WHERE sequence_schema = 'public';" "$db")
...
compare_views() {
    ...
    for db in $dbs; do
        ...
        local new_views=$(run_sql $NEW_HOST "SELECT viewname, definition FROM pg_views WHERE schemaname = 'public';" "$db")
...
compare_triggers() {
    ...
    for db in $dbs; do
        ...
        local new_triggers=$(run_sql $NEW_HOST "SELECT tgname, tgrelid::regclass, pg_get_triggerdef(oid) FROM pg_trigger WHERE tgisinternal = false;" "$db")
...
compare_constraints() {
    ...
    for db in $dbs; do
        ...
        local new_constraints=$(run_sql $NEW_HOST "SELECT conname, contype, conrelid::regclass, pg_get_constraintdef(oid) FROM pg_constraint WHERE connamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');" "$db")
