...
properties(
	[
		parameters([
			...
			string(defaultValue: 'mongodb.mongodb', name: 'stringPattern'),
		])
	]
)
...
currentBuild.displayName = "#${BUILD_NUMBER}-${targetEnvironmentName}-docdb-verification"
def podsContainers = []
	...
	container('tool') {
		dir(tempDir){
			...
			stage("Verifying Services") {
				...
				for (podContainer in podsContainers) {
					if (podContainer.contains("sink")) {
						def (pod, container) = podContainer.split('---')
						
						if (pod && container) {
							sh"""
								kubectl -n justice exec -t ${pod.trim()} -c ${container.trim()} -- curl http://localhost:8083/connectors/account-history-sink > account-history-sink.txt
							"""

							def sinkOutput = readFile("account-history-sink.txt")

							if (sinkOutput.contains("mongodb.mongodb")) {
							sh """
									echo "Re-create connector sink"
									kubectl -n justice exec -t ${pod.trim()} -c ${container.trim()} -- sh <<EOF
curl -v http://localhost:8083/connectors/documentdb-sink -X DELETE
curl -v http://localhost:8083/connectors/account-history-sink -X DELETE
/bin/bash /tmp/sink/connector.sh
EOF
"""
							}							
						}
