...
output_file="DbTablePreflightReport.txt"
...
check_karpenter_version() {
    ...
    if [[ -z $current_version ]]; then
        echo "This env not using Karpenter (check skipRolloutCriticalOnInit)"
    else
...
find_critical_on_init_asg() {
    ...
    --query 'AutoScalingGroups[?length(Instances) >= `2` && contains(AutoScalingGroupName, `'"$env-critical-on-init"'`)].AutoScalingGroupName' \
    --output text)
    
    if [[ -z $asg_name ]]; then
        echo "This env doesn't have asg critical-on-init (check skipRolloutCriticalOnInit)"
    else
        echo $asg_name
    fi   
}
...
describe_ami_asg(){
    ...
    if [[ -z $ami_version ]]; then
        echo "This env not doesn't have asg critical-on-init (check skipRolloutCriticalOnInit)"
    else
...
check_armada_services() {
    ...
    if [[ $consul_server -ne 0 && $nomad_server -ne 0 ]]; then
        echo "❌ Armada services are not available (checklist skipRolloutArmadaServices)";
    else 
        echo "✅ Armada services are available (uncheck skipRolloutArmadaServices and backup necessary configuration)";
    fi
}
...
check_extend_services() {
    ...
    if [[ "$crossplane_deployment" == true && "$extend_namespace" == true ]]; then
        echo "✅ Extend services are available (uncheck skipRolloutExtendServices)"
    else
        echo "❌ Extend services are not available (checklist skipRolloutExtendServices)"
    fi
}
...
check_tg_state() {
    local name=$1
    local state_file=$2
    readstatefile=$(cat $state_file)

    if echo "$readstatefile" | grep -q "$name"; then
        echo "✅ found state addon $name"
    else
        echo "⚠️ not found state addon $name"
    fi
}

check_error_sts_services() {
    kubectl get sts -A  --no-headers | awk '{ split($3, a, "/"); if (a[1] != a[2]) print $0 }' | grep -Ev 'Running|Completed' | awk {'print $2" "$3'}
}
...
print_table() {
    {
        ...
        printf ""
        {
            echo "Pre-flight Report for environment: $env"
            printf "+------------------------------------------------------------------------------------------------------------------------------------------+\n"
            printf "| %-15s | %-35s | %-80s |\n" "Component" "Criteria" "Result"
            printf "+------------------------------------------------------------------------------------------------------------------------------------------+\n"

            if [[ "$component" == "addon" ]]; then
                check_file "terragrunt_state_list.txt"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Min Control Plane Version        " "$(check_k8s_control_plane "$min_eks_version")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Cluster Health Issue             " "$(check_cluster_insight "Cluster")"
                ...
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Addons vpc-cni                   " "$(check_k8s_addon "vpc-cni" "$(k8s_yaml_parser "$min_eks_version" "ADDONS.VPC_CNI_VERSION")")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Addons kube-proxy                " "$(check_k8s_addon "kube-proxy" "$(k8s_yaml_parser "$min_eks_version" "ADDONS.KUBE_PROXY_VERSION")")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Addons aws-ebs-csi-driver        " "$(check_k8s_addon "aws-ebs-csi-driver" "$(k8s_yaml_parser "$min_eks_version" "ADDONS.EBS_CSI_VERSION")")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Addons coredns                   " "$(check_k8s_addon "coredns" "$(k8s_yaml_parser "$min_eks_version" "ADDONS.COREDNS_VERSION")")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Tg state vpc-cni                 " "$(check_tg_state "vpc-cni" "terragrunt_state_list.txt")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Tg state kube-proxy              " "$(check_tg_state "kube-proxy" "terragrunt_state_list.txt")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Tg state aws-ebs-csi-driver      " "$(check_tg_state "ebs-csi-driver" "terragrunt_state_list.txt")"
                printf "| %-15s | %-35s | %-85s \n" "Addons" "Tg state coredns                 " "$(check_tg_state "coredns" "terragrunt_state_list.txt")"
            fi

            if [[ "$component" == "controlplane" ]]; then
                printf "| %-15s | %-35s | %-85s \n" "Control Plane" "Min Control Plane Version " "$(check_k8s_control_plane "$min_eks_version")"
            ...
            if [[ "$component" == "workernode" ]]; then
                ...
                printf "| %-15s | %-35s | %-85s \n" "Worker Node" "Min Worker Node Version     " "$(check_k8s_worker_node "$min_eks_version")"
                ...
                printf "| %-15s | %-35s | %-85s \n" "Worker Node" "Falco Version               " "$(check_falco_version "$(k8s_yaml_parser "$eks_version" "FALCO_VERSION")")"
                ...
                if [[ -z "$error_list_sts" ]]; then
                    ...
                    printf "| %-15s | %-35s | ⚠️  %-77s \n" "Worker Node" "Error Statefulset" "stsNamespace stsNotReady"
