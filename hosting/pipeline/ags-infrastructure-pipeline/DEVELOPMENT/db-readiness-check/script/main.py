...
available_test_cases = [test_cases[v]['resource_type'] for v in range(len(test_cases)) if test_cases[v]['resource_type'] not in case_exists and (case_exists.add(test_cases[v]['resource_type']) or True)]
single_table_test_cases = json.load(open('{}/config.json'.format(script_directory), 'r'))['single_table']

for c in single_table_test_cases:
    if c in available_test_cases:
        available_test_cases.remove(c)
...
for resource_type in available_test_cases:
    ...
    try:
        ...
        if resource_type == 'rds':
            resource_list = [{"index": x, "name": sp_output['rows'][x]['resource_name']} for x in range(len(sp_output['rows'])) if re.match("rds-.*-justice-{}.*".format(environment_name), sp_output['rows'][x]['resource_name'])]
        else:
            resource_list = [{"index": x, "name": sp_output['rows'][x]['resource_name']} for x in range(len(sp_output['rows'])) if re.match(".*{}-justice-{}.*".format(customer_name, environment_name), sp_output['rows'][x]['resource_name'])]
    ...
    for r in range(len(resource_list)):
        ...
        table_header = ['Criteria', 'Current', 'Expected', 'Status']

        csv_file = open('result/{}_{}.csv'.format(resource_type, resource_name), 'w')
        csv_output_writer = csv.writer(csv_file)
        csv_output_writer.writerow(table_header)

        t = PrettyTable(table_header, align='l')
            ...
            for idx, resource_case in enumerate(resource_test_cases_keys):
                try:
                    ...
                    if rule_engine == 'regex':
                        ...
                        t.add_row([key_label, current_value, expected_value, evaluator])
                        ...
                        t.add_row([key_label, current_value, expected_value, evaluator])
                        csv_output_writer.writerow([key_label, str(current_value), str(expected_value), evaluator])
                        ...
                        if evaluator is None:
                            t.add_row([key_label, evaluator['result'], "TEST-CASE-ERROR", False])
                            csv_output_writer.writerow([key_label, evaluator['result'], "TEST-CASE-ERROR", False])
                        else:
                            t.add_row([key_label, evaluator['result'], expected_value, evaluator['status']])
                            csv_output_writer.writerow([key_label, evaluator['result'], expected_value, evaluator['status']])
                    ...
                    if key_label is not None:
                        t.add_row([key_label, "TEST-CASE-ERROR", "TEST-CASE-ERROR", False])
                        csv_output_writer.writerow([key_label, current_value, expected_value, evaluator])
                        failed_test_case.append(resource_case)
                    elif resource_case is not None:
                        t.add_row([resource_case, "TEST-CASE-ERROR", "TEST-CASE-ERROR", False])
        ...
        # Print output
        print("- ", sp_output['rows'][resource_name_idx]['resource_name'])
        print(t)
        print("")
